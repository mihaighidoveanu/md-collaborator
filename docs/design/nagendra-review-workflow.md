...
- `two_way` → render a **read‑first diff panel** above/around the editor showing
  ...
  looked").
- `three_way` → render a **3‑column** view: **Base | Upstream (author) | Your
  edits**, aligned by the `diff` rows from `threeWay`. Highlight cells differing
  from base; mark `conflict` rows distinctly (amber). Offer per‑file actions:
  **"Keep mine"**, **"Take upstream"**, or **"Edit merged"** (loads a chosen side
  into the editor to hand‑merge). Whatever the reviewer lands on becomes the
  editor content and autosaves as usual.
